...
>
    ...
    <div class="absolute inset-0 z-[1] overflow-hidden pointer-events-none">
        <div class="absolute -top-40 -right-40 w-[500px] h-[500px] bg-accent/30 rounded-full blur-[120px] animate-float"></div>
        <div class="absolute -bottom-40 -left-40 w-[400px] h-[400px] bg-accent-2/20 rounded-full blur-[100px] animate-float" style="animation-delay: -2s;"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-r from-accent/10 to-accent-2/10 rounded-full blur-[80px] animate-pulse"></div>
    </div>

    {{-- Grid pattern overlay --}}
    <div class="absolute inset-0 z-[2] opacity-[0.03]" style="background-image: linear-gradient(var(--color-text) 1px, transparent 1px), linear-gradient(90deg, var(--color-text) 1px, transparent 1px); background-size: 60px 60px;"></div>

    {{-- Gradient Overlay --}}
    <div class="absolute inset-0 bg-gradient-to-b from-[var(--color-bg-primary)]/50 via-transparent to-[var(--color-bg-primary)] z-[3]"></div>
    ...
    <div class="container relative z-10 pt-32 pb-20">
        <div class="max-w-5xl mx-auto text-center">
            ...
            <p
                class="text-lg md:text-xl lg:text-2xl text-text-muted mb-12 max-w-3xl mx-auto leading-relaxed transition-all duration-1000 delay-200"
                :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
            >
                {{ $subtitle }}
            </p>
            ...
            {{-- CTAs --}}
            <div
                class="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center transition-all duration-1000 delay-300"
                :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
            >
                @foreach($ctas as $cta)
                    ...
                    @else
                        ...
                        >
                            <span class="absolute inset-0 rounded-full border-2 border-white/20 group-hover:border-accent transition-colors duration-300"></span>
                            <span class="absolute inset-0 rounded-full bg-white/0 group-hover:bg-white/5 transition-colors duration-300"></span>
            ...
            {{-- Trust badges --}}
            <div
                class="mt-16 pt-16 border-t border-white/5 transition-all duration-1000 delay-500"
                :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
            >
                ...
                <div class="flex flex-wrap justify-center items-center gap-8 md:gap-12 opacity-50">
                    @foreach(['Google', 'Microsoft', 'Apple', 'Meta', 'Amazon'] as $company)
                        <span class="text-lg font-semibold text-text-muted hover:text-white transition-colors">{{ $company }}</span>
                    @endforeach
                </div>
            </div>
        </div>
    ...
    >
        <span class="text-xs text-text-muted uppercase tracking-widest mb-3 group-hover:text-accent transition-colors">Scroll to explore</span>
        <div class="w-6 h-10 border-2 border-white/20 rounded-full flex justify-center pt-2 group-hover:border-accent/50 transition-colors">
