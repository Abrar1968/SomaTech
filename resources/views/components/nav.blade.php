...
<nav x-data="{
    ...
    class="fixed top-0 left-0 right-0 z-50 transition-all duration-500" role="navigation" aria-label="Main navigation">
    {{-- Top accent line --}}
    <div
        class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-accent to-transparent opacity-50">
    </div>
    ...
    <div class="container">
        <div class="flex items-center justify-between h-20 lg:h-24">
            ...
            <a href="{{ route('home') }}" class="relative flex items-center gap-3 group z-10">
                <div class="relative">
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-accent to-accent-2 rounded-xl blur-lg opacity-50 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    <div
                        class="relative w-11 h-11 bg-gradient-to-br from-accent to-accent-2 rounded-xl flex items-center justify-center transform group-hover:scale-105 group-hover:rotate-3 transition-all duration-500 shadow-lg">
                ...
                <div class="flex flex-col">
                    ...
                    <span
                        class="text-[10px] text-text-muted uppercase tracking-[0.2em] font-medium -mt-1 hidden sm:block">Digital
                        Agency</span>
                </div>
            </a>
            ...
            <div class="hidden lg:flex items-center">
                <div
                    class="flex items-center gap-1 bg-[var(--color-bg-surface)]/50 backdrop-blur-sm rounded-full px-2 py-2 border border-white/5">
                ...
                <a href="{{ route('contact.index') }}"
                    ...
                    <span
                        class="absolute inset-0 bg-gradient-to-r from-accent to-accent-2 transition-transform duration-500 group-hover:scale-105"></span>
                    <span
                    ...
                    <span
                        class="absolute inset-[2px] bg-[var(--color-bg-primary)] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
                    <span
                        class="relative z-10 flex items-center gap-2 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-accent group-hover:to-accent-2 group-hover:bg-clip-text">
                        Let's Talk
            ...
            <button @click="mobileOpen = !mobileOpen"
                ...
                :aria-expanded="mobileOpen" aria-controls="mobile-menu" aria-label="Toggle navigation menu">
                <div class="w-5 h-4 flex flex-col justify-between">
                    <span :class="mobileOpen ? 'rotate-45 translate-y-[7px]' : ''"
                        class="block h-[2px] w-full bg-white rounded-full transition-all duration-300 origin-center"></span>
                    <span :class="mobileOpen ? 'opacity-0 scale-0' : ''"
                        class="block h-[2px] w-full bg-white rounded-full transition-all duration-300"></span>
                    <span :class="mobileOpen ? '-rotate-45 -translate-y-[7px]' : ''"
                        class="block h-[2px] w-full bg-white rounded-full transition-all duration-300 origin-center"></span>
                </div>
            </button>
    ...
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-400" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-300"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="mobileOpen = false"
        class="lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-40" x-cloak></div>

    {{-- Mobile Menu Panel --}}
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-400"
        ...
        <div class="p-8 pt-28">
            ...
            <nav class="space-y-2">
                ...
                @auth
                    <x-mobile-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.*')">Admin Panel</x-mobile-nav-link>
                @else
                    <x-mobile-nav-link :href="route('login')" :active="request()->routeIs('login')">Admin Login</x-mobile-nav-link>
                @endauth
            </nav>
            ...
            <div class="mt-8">
                ...
                <div class="flex gap-4">
                    <a href="#"
                        class="w-10 h-10 rounded-xl bg-[var(--color-bg-surface)] flex items-center justify-center text-text-muted hover:text-accent hover:bg-accent/10 transition-all"
                        aria-label="LinkedIn">
