<x-layouts.admin title="Edit Team Member">
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold font-display">Edit Team Member</h1>
                <p class="text-text-muted mt-1">{{ $teamMember->name }}</p>
            </div>
        </div>

        <form action="{{ route('admin.team.update', $teamMember) }}" method="POST" enctype="multipart/form-data" class="grid lg:grid-cols-3 gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <input
                                ...
                                name="github_url"
                                id="github_url"
                                value="{{ old('github_url', $teamMember->github_url) }}"
                                class="w-full px-4 py-3 bg-[var(--color-bg-primary)] border border-white/10 rounded-xl focus:outline-none focus:border-accent transition-colors"
                ...
                <div class="bg-[var(--color-bg-surface)] rounded-2xl border border-white/5 p-6 space-y-5">
                    ...
                    <div x-data="{ tags: {{ json_encode(old('skills', $teamMember->skills ?? [])) }}, newTag: '' }" class="space-y-3">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-[var(--color-bg-surface)] rounded-2xl border border-white/5 p-6 space-y-5">
                    ...
                    <div
                        x-data="{ preview: '{{ $teamMember->photo ? Storage::url($teamMember->photo) : '' }}' }"
                        class="space-y-3"
                    >
                        <div
                ...
                <div class="bg-red-500/5 rounded-2xl border border-red-500/10 p-6 space-y-4">
                    ...
                    <p class="text-sm text-text-muted">Permanently delete this team member.</p>
                    <form action="{{ route('admin.team.destroy', $teamMember) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Are you sure you want to delete this team member?')" class="w-full px-4 py-2.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl hover:bg-red-500/20 transition-colors font-medium">
                            Delete Team Member
                        </button>
                    </form>
                </div>
